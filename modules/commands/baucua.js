...
var full = [];
    ...
    attachment: (await require('axios').get(`https://i.imgur.com/YgccybN.jpeg`, {
        responseType: 'stream'
            ...
            if (moneyBet < 10000) return api.sendMessage("[💵]➜ Số tiền đặt không được dưới: 10000$", event.threadID, event.messageID);
    ...
    attachment: (await require('axios').get(`https://i.imgur.com/YgccybN.jpeg`, {
        responseType: 'stream'
            ...
            api.sendMessage({body:"[🐧]➜ Vui lòng chờ kết quả!",attachment: createReadStream(__dirname + "/cache/baucua/baucua.gif")},event.threadID,async (error,info) => {
            ...
            if (i == 1) {
                ...
                var mon = parseInt(args[1]) * 3; 
                    await Currencies.increaseMoney(event.senderID, mon); console.log('s3')
                        return api.sendMessage({body:`[🐧]=== 『 ALL FILE 』 ===[🐧]\n◆━━━━━━━━━━━━━━━━◆\n\n[🐧]➜ Kết quả: ${full.join("•")}\n[🐧]➜ Có 3 ${args[0].toLocaleLowerCase()} ${icon}\n[🐧]➜ Bạn chọn: ${args[0].toLocaleLowerCase()}\n[🐧]➜ Bạn đã thắng: +${mon}$\n💰 Số dư hiện tại là: ${[moneyUser + mon]}$`,attachment: listimg},event.threadID, event.messageID);
            }
