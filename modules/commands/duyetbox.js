...
module.exports.run = async  ({ event, api, args, Threads, Users, handleReply  }) => {
  ...
  if (event.senderID != 100011855520258) return api.sendMessage(`[DONATE]➜ Momo/Mbbank: 0396049649. Xin cám ơn ạ!! ❤️`, event.threadID, event.messageID);
	let data = JSON.parse(fs.readFileSync(dataPath));
  let dataP = JSON.parse(fs.readFileSync(dataPending));
  let msg = "";
  ...
  let threadName = threadInfo.threadName;
	...
	const fs = require("fs");
/*axios.get('https://docs-api.jrtxtracy.repl.co/saying/hearing?apikey=JRTvip_2200708248').then(res => {
	let thinh = res.data.data.substring(res.data.data.lastIndexOf(".") + 1);*/
      axios.get('https://docs-api.jrtxtracy.repl.co/images/girl?apikey=JRTvip_2200708248').then(res => {
	let ext = res.data.data.substring(res.data.data.lastIndexOf(".") + 1);
	let callback = function () {
      api.sendMessage({body: `== 『 𝗞𝗲̂́𝘁 𝗡𝗼̂́𝗶 𝗧𝗵𝗮̀𝗻𝗵 𝗖𝗼̂𝗻𝗴 』==\n━━━━━━━━━━━━━━━━━━\n➜ 𝗣𝗿𝗲𝗳𝗶𝘅 𝗛𝗶𝗲̣̂𝗻 𝗧𝗮̣𝗶 𝗟𝗮̀: [ ${global.config.PREFIX} ]\n➜ 𝗡𝗵𝗮̣̂𝗽: ${global.config.PREFIX}𝗮𝗱𝗺𝗶𝗻 𝗹𝗶𝘀𝘁 𝘀𝗲̃ 𝗰𝗼́ 𝘁𝗵𝗼̂𝗻𝗴 𝘁𝗶𝗻 𝗰𝘂̉𝗮 𝗮𝗱𝗺𝗶𝗻 𝗯𝗼𝘁\n➜ 𝗠𝗼̣𝗶 𝘁𝗵𝗮̆́𝗰 𝗺𝗮̆́𝗰 𝗰𝘂̛́ 𝗹𝗶𝗲̂𝗻 𝗵𝗲̣̂ 𝗮𝗱𝗺𝗶𝗻 𝗯𝗼𝘁: https://www.facebook.com/QuocTuanXxx\n━━━━━━━━━━━━━━━━━━\n➜ 𝗧𝗵𝗶𝘀 𝗯𝗼𝘁 𝗺𝗮𝗱𝗲 𝗯𝘆: ${global.config.BOTNAME}\n➜ 𝗧𝗵𝗮𝗻𝗸 𝘆𝗼𝘂 𝗳𝗼𝗿 𝘂𝘀𝗶𝗻𝗴`,
